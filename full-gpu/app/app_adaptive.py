...
def list_anomaly_clips():
    try:
        clips = []
        for f in sorted(ANOMALY_CLIPS_DIR.glob("*.mp4"), reverse=True):
            p = f.stem.split('_')
            if len(p) >= 6:
                clips.append({"clip_id": p[5], "timestamp": f"{p[1][:4]}-{p[1][4:6]}-{p[1][6:8]} {p[2][:2]}:{p[2][2:4]}:{p[2][4:6]}", "severity": p[3], "type": p[4], "size": f"{f.stat().st_size/(1024*1024):.1f} MB"})
